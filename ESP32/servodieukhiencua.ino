...
void handleRainSensor() {
    ...
    if (mqttWindowOverride) {
        Serial.println("Rain Sensor: MQTT override is active. Skipping auto control this cycle.");
        mqttWindowOverride = false; // Reset cờ. MQTT chỉ ưu tiên 1 chu kỳ.
        // Vẫn publish trạng thái mưa nếu có thay đổi
        if (isRaining != Previousrainsensor) {
            publishRainDetectedState(isRaining);
        }
        return; // Không thực hiện logic tự động khi có override
    }
